...
function DrawerAppBar(props) {
  ...
  const [Tokprice, setTokprice] = React.useState('');
  const [Tokinput, setTokinput] = React.useState('');
  const [periodindex, setPeriodindex] = React.useState('');
  const [currentAccount, setCurrentAccount] = React.useState('');
  const [Bros, setBros] = React.useState('');
  const [TokBal, setTokBal] = React.useState('');
...
async function Checkfee(value) {
  try {
      ...
      if (ethereum) {
          ...
          let tx = await contract.getTokensAmount(ethers.parseUnits(value ? value.toString() : "0", 18));
          //await tx.wait();
...
async function Period2() {
  setPeriodindex(1)
};
/*async function Period3() {
  setPeriodindex(2)
};
async function Period4() {
  setPeriodindex(3)
};*/
...
async function Tokbal1() {
  try {
      ...
      if (ethereum) {
          ...
          //await tx.wait();
          console.log("exchange", ethers.formatUnits(tx, 18))
          setTokBal(ethers.formatUnits(tx, 18))
      }
    } catch(error) {
      console.log(error);
    }
...
const GetTokens = async () => {
  try {
    ...
    if (ethereum) {
        ...
        const contract = new ethers.Contract(CONTRACT_ADDRESS, contractAbi, signer)
        let tx = await contract.buy(Bros ? Bros : currentAccount, periodindex ? periodindex : 0, {value: ethers.parseEther(Tokinput)});
        await tx.wait();
    }
  } catch(error) {
    console.log(error);
              ...
              <div className='scroll1'>
                ...
                <Card className='t2'>
                  <div className='innercard'>
                    <Typography className='t3'>New Bond</Typography>
                    <div className='items2'>
                      ...
                      <div className='imputu'>
                        ...
                        <TextField id="outlined-basic" className='t8' label={Tokprice ? Tokprice : 0} disabled={disabled} variant="outlined" />
                    ...
                    <div className='selectto'>
                      ...
                      <Button variant="contained" className='t6 none' >20 days</Button>
                      <Button variant="contained" className='t6 none' >10 days</Button>
                      <Button variant="contained" className='t6 none' >5 days</Button>
                    </div>
                    <Button variant="contained" className='t4' onClick={GetTokens}>Get Tokens</Button>
